	...
	<form method="post" action='' id="convert_post">
		...
		<select name="post_from">
			<?php foreach ($post_type as $types) { 
				if($types->name!='attachment' && $types->name!='revision' && $types->name!='nav_menu_item' && $types->name!='custom_css' && $types->name!='customize_changeset') {?>
		...
		<br>
		<br>
		Give your rating and reviews on <a href="https://wordpress.org/support/plugin/copy-move-posts/reviews/" target="_blank">wordpress.org</a> to spread the love.
	</form>
</div>
